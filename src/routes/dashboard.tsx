...
export const Route = createFileRoute("/dashboard")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Painel de controle para médicos: agendamentos, pacientes, faturamento e métricas em tempo real." },
    ],
  }),
  component: Dashboard,
});
...
const todayAppointments: Appt[] = [
  { id: "1", time: "08:30", patient: "Mariana Costa", age: 34, type: "Presencial", reason: "Consulta de rotina", status: "concluido", avatar: "MC" },
  { id: "2", time: "09:15", patient: "Rafael Almeida", age: 52, type: "Presencial", reason: "Retorno — hipertensão", status: "concluido", avatar: "RA" },
  { id: "3", time: "10:00", patient: "Beatriz Lima", age: 28, type: "Teleconsulta", reason: "Avaliação cardiológica", status: "em-andamento", avatar: "BL" },
  { id: "4", time: "11:30", patient: "João Pedro Santos", age: 41, type: "Presencial", reason: "Eletrocardiograma", status: "confirmado", avatar: "JS" },
  { id: "5", time: "14:00", patient: "Carla Mendes", age: 60, type: "Presencial", reason: "Holter 24h — entrega", status: "confirmado", avatar: "CM" },
  { id: "6", time: "15:45", patient: "Tiago Ribeiro", age: 37, type: "Teleconsulta", reason: "Resultado de exames", status: "aguardando", avatar: "TR" },
  { id: "7", time: "16:30", patient: "Luana Ferreira", age: 45, type: "Presencial", reason: "Consulta inicial", status: "confirmado", avatar: "LF" },
  { id: "8", time: "17:15", patient: "Eduardo Nunes", age: 29, type: "Teleconsulta", reason: "Acompanhamento", status: "cancelado", avatar: "EN" },
];
...
const stats: Stat[] = [
  { label: "Consultas hoje", value: "8", delta: "+12,5%", trend: "up", icon: CalendarDays, iconBg: "bg-teal-50", iconText: "text-teal-600" },
  { label: "Pacientes ativos", value: "342", delta: "+8,2%", trend: "up", icon: Users, iconBg: "bg-indigo-50", iconText: "text-indigo-600" },
  { label: "Faturamento (mês)", value: "R$ 48.720", delta: "+18,4%", trend: "up", icon: Wallet, iconBg: "bg-emerald-50", iconText: "text-emerald-600" },
  { label: "Taxa de no-show", value: "4,2%", delta: "-1,1%", trend: "down", icon: Activity, iconBg: "bg-amber-50", iconText: "text-amber-600" },
];
...
const statusStyle: Record<Appt["status"], { label: string; bg: string; text: string; dot: string }> = {
        ...
        <aside className="hidden lg:flex w-64 shrink-0 flex-col border-r border-slate-200 bg-white min-h-screen sticky top-0">
          ...
          <div className="p-4 border-t border-slate-100">
            <div className="flex items-center gap-3 p-2 rounded-lg hover:bg-slate-50 transition">
              <div className="h-10 w-10 rounded-full bg-gradient-to-br from-teal-500 to-indigo-600 grid place-items-center text-white font-semibold">DR</div>
              ...
              <div className="xl:col-span-2 rounded-2xl bg-white border border-slate-200 overflow-hidden">
                <div className="px-6 py-5 border-b border-slate-100 flex items-center justify-between gap-4 flex-wrap">
                  <div>
                    ...
                    <p className="text-xs text-slate-500 mt-0.5">{todayAppointments.length} consultas programadas</p>
                  </div>
                ...
                <ul className="divide-y divide-slate-100">
                  {filtered.map((a) => {
                    ...
                    return (
                      <li key={a.id} className="px-6 py-4 hover:bg-slate-50/60 transition flex items-center gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            <span className="text-sm font-medium text-slate-900 truncate">{a.patient}</span>
                        ...
                        <span className={`hidden sm:inline-flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-full ${st.bg} ${st.text}`}>
                  ...
                  {filtered.length === 0 && (
                    <li className="px-6 py-12 text-center text-sm text-slate-500">Nenhuma consulta neste filtro.</li>
                  )}
                </ul>
              </div>
              ...
              <div className="space-y-6">
                <div className="rounded-2xl bg-gradient-to-br from-slate-900 via-slate-900 to-indigo-950 text-white p-6 relative overflow-hidden">
                  ...
                  <div className="relative">
                    ...
                    <p className="text-sm text-slate-300 mt-1">Avaliação cardiológica · Teleconsulta</p>
                ...
                <div className="rounded-2xl bg-white border border-slate-200 p-6">
                  ...
                  <ul className="mt-4 space-y-3 text-sm">
                    <Row icon={CheckCircle2} color="text-emerald-600" label="Consultas concluídas" value="2" />
                    <Row icon={Clock} color="text-amber-600" label="Tempo médio por consulta" value="28 min" />
                    <Row icon={XCircle} color="text-rose-600" label="Cancelamentos" value="1" />
                    <Row icon={DollarSign} color="text-teal-600" label="Receita prevista hoje" value="R$ 3.240" />
                    <Row icon={Phone} color="text-indigo-600" label="Retornos agendados" value="4" />
                  </ul>
                </div>
              </div>
            </section>
              ...
              <div className="xl:col-span-2 rounded-2xl bg-white border border-slate-200 p-6">
                <div className="flex items-center justify-between mb-6">
                  ...
                  <div className="text-xs text-slate-500">
                    Total: <span className="font-semibold text-slate-900">{weekData.reduce((s, d) => s + d.consultas, 0)}</span>
                  </div>
                </div>
...
function Row({ icon: Icon, color, label, value }: { icon: LucideIcon; color: string; label: string; value: string }) {
  return (
